...
class Generator(object):
    ...
    def search_url(self):
        if self.history.current_keyword_num >= len(keywords):
            return
    ...
    def page_url(self, keyword):
        # print 'comein'
        r = self.reqst.get("http://xueqiu.com/")#, headers={"user-agent": "Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/45.0.2454.93 Safari/537.36"})
        ...
        # print url
        jscontent = self.reqst.get(url)#, headers={"user-agent": "Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/45.0.2454.93 Safari/537.36"})#, "cookie": current_cookie}).text
        # print jscontent.decode('utf8')
        js_dict = json.loads(jscontent.text)
