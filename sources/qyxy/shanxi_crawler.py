...
class ShanxiCrawler(Crawler):
    ...
    urls = {'host': 'www.hljaic.gov.cn',
            ...
            'post_checkCode2': 'http://218.26.1.108/searchList.jspx',
            'ind_comm_pub_skeleton': 'http://218.26.1.108/businessPublicity.jspx?',
            'ent_pub_skeleton': 'http://218.26.1.108/enterprisePublicity.jspx?',
            'other_dept_pub_skeleton': 'http://218.26.1.108/otherDepartment.jspx?',
            'judical_assist_skeleton': 'http://218.26.1.108/justiceAssistance.jspx?',
...
class ShanxiParser(Parser):
        ...
        for tr in table_trs[2:]:
            ...
            if a:
                for a1 in a:
                    ...
                    args = m.group(1)
                    host = 'http://218.26.1.108/QueryYearExamineDetail.jspx?id='
                    url = '%s%s' % (host, args)
                    rep = requests.get(url)
    ...
    def get_shareholder_detail(self, content_tr):
        ...
        int1 = m.group(1)
        url1 = '%s%s' %('http://218.26.1.108/queryInvDetailAction.jspx?id=', int1)
    ...
    def get_movable_property_reg_detail(self, content_tr):
        ...
        m = re.search(r'id=(.\d+)', str(link))
        int1 = m.group(1)
        url1 = '%s%s' %('http://218.26.1.108/mortInfoDetail.jspx?id=', int1)
        rep1 = requests.get(url1)
        soupn = BeautifulSoup(rep1.text, "html5lib")
...
if __name__ == '__main__':
    ...
    # enterprise_list = ['141000000031826']
