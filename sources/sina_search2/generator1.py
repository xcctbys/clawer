...
logfilename = '%s/%s%s%s' % ('/data/media/tmp/logs', 'sinasearch_generator_test', '_'.join(str(datetime.date.today()).split('-')), '.log')
...
class Generator(object):
    ...
    def page_url(self, keyword, media = ''):
        ...
        logging.warning('page url %s'%url)
    ...
    def do_obtain(self, count, url):
        for i in range(1, ((count-self.history.current_url_num)/10)+2):
            if i > 1:
                self.flag = False
                return
            #col=1_7 限定财经频道
            ...
            logging.warning('pageurl %s'%page)
            try:
